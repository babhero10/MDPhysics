...
class MotionDPT(nn.Module):
    def __init__(self, dim, cfg):
        ...
        if hasattr(cfg, "backbone_name"):
            backbone_name = cfg.backbone_name
        if hasattr(cfg, "depth_backbone_name"):
            depth_name = cfg.depth_backbone_name
        if hasattr(cfg, "out_indices"):
            self.out_indices = cfg.out_indices

        # Load Backbones
        self.backbone = AutoModel.from_pretrained(backbone_name)

        if cfg.depth_backbone:
            self.depth_model = AutoModelForDepthEstimation.from_pretrained(
                depth_name, trust_remote_code=True
            )

        # Freeze Backbones
        freeze_backbones = True
        if hasattr(cfg, "freeze_backbones"):
            freeze_backbones = cfg.freeze_backbones
        elif isinstance(cfg, dict) and "freeze_backbones" in cfg:
            freeze_backbones = cfg["freeze_backbones"]
        ...
        self.fusion_blocks.append(
            FusionBlock(self.dim, polar_patch_size, struct_resolutions[1])
        )
        self.fusion_blocks.append(
            FusionBlock(self.dim, polar_patch_size, struct_resolutions[2])
        )
        self.fusion_blocks.append(
            FusionBlock(self.dim, polar_patch_size, struct_resolutions[3])
        )
    ...
    def forward(self, x):
        ...
        if cfg.depth_backbone:
            ...
            if depth.shape[-2:] != x_t.shape[-2:]:
                depth = F.interpolate(
                    depth, size=x_t.shape[-2:], mode="bilinear", align_corners=False
                )
