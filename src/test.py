    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Testing"):
            ...
            with torch.amp.autocast(device_type):
                outputs = model(blur, depth=depth)
            ...
            if "sharp_image" in outputs:
                ...
                for i in range(batch_size):
                    ...
                    # Determine number of subplots based on depth availability
                    n_cols = 5 if depth is not None else 4
                    ...
                    # Input Depth (if available)
                    if depth is not None:
                        depth_np = depth[i, 0].cpu().numpy()
                        im_depth = axes[4].imshow(depth_np, cmap="magma")
                        axes[4].set_title("Input Depth")
                        axes[4].axis("off")
                        plt.colorbar(im_depth, ax=axes[4], fraction=0.046, pad=0.04)
